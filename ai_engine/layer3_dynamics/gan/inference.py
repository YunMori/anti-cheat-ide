...
class GANInference:
    def __init__(self, model_path: Optional[str] = None, config: dict = None):
        ...
        if model_path and Path(model_path).exists():
            ...
            self.trained = True
            logger.info(f"Loaded GAN generator from {model_path} (noise_dim={self.noise_dim})")
        else:
            logger.info(f"No GAN model found at {model_path}, using untrained generator (fallback to HMM)")
